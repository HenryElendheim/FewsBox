...
/**
 * Dead-simple persistence: battle progress, party pick and every hero's
 * equipment go into SharedPreferences. Everything loaded is validated
 * against current content, so removed or renamed gear falls back to the
 * hero's defaults instead of crashing an old save.
 */
object SaveStore {
    ...
    fun load(context: Context): SaveData {
        ...
        val roster = Party.rosterDefaults().map { loadout ->
        ...
        return SaveData(battleIndex, selected, roster)
    }

    fun save(context: Context, battleIndex: Int, selectedIds: Set<String>, roster: List<Loadout>) {
